const Joi = require('joi')

const validateTestMarks = (value, helpers) => {
  const totalQ = Number(value.totalQuestions || 0)
  const marksPerQ = Number(value.marksPerQuestion || 1)
  const expectedTotalMarks = totalQ * marksPerQ

  if (value.totalMarks === undefined || value.totalMarks === null || Number(value.totalMarks) === 0) {
    value.totalMarks = expectedTotalMarks
  } else if (Number(value.totalMarks) !== expectedTotalMarks) {
    return helpers.message(`Total marks must be equal to Total Questions * Marks Per Question (${totalQ} * ${marksPerQ} = ${expectedTotalMarks})`)
  }

  const passing = Number(value.passingMarks || 0)
  if (passing > Number(value.totalMarks)) {
    return helpers.message(`Passing marks (${passing}) cannot be greater than Total marks (${value.totalMarks})`)
  }

  return value
}

const createCourseTestSchema = Joi.object({
  course: Joi.string().hex().length(24).optional(),
  courseId: Joi.string().hex().length(24).optional(),
  subjects: Joi.array().items(Joi.string().hex().length(24)).optional(),
  topics: Joi.array().items(Joi.string().hex().length(24)).optional(),
  chapters: Joi.array().items(Joi.string().hex().length(24)).optional(),
  masterIds: Joi.array().items(
    Joi.object({
      courseId: Joi.string().hex().length(24).required(),
      subjectId: Joi.string().hex().length(24).optional().allow(null, '', 'null', 'undefined'),
      chapterId: Joi.string().hex().length(24).optional().allow(null, '', 'null', 'undefined'),
      topicId: Joi.string().hex().length(24).required(),
    })
  ).optional(),
  title: Joi.string().trim().required(),
  slug: Joi.string().trim(),
  description: Joi.string().trim().optional().allow('', null),
  instruction: Joi.string().trim().optional().allow('', null),
  instructionsNew: Joi.string().trim().optional().allow('', null),
  image: Joi.string().trim().optional().allow('', null),
  duration: Joi.number().empty(['', 'null', 'undefined', null]).min(1).required(),
  sortOrder: Joi.number().empty(['', 'null', 'undefined', null]).integer().min(0).default(0),
  totalQuestions: Joi.number().empty(['', 'null', 'undefined', null]).min(0).default(0),
  totalMarks: Joi.number().empty(['', 'null', 'undefined', null]).min(0).default(0),
  passingMarks: Joi.number().empty(['', 'null', 'undefined', null]).min(0).default(0),
  marksPerQuestion: Joi.number().empty(['', 'null', 'undefined', null]).min(1).default(1),
  negativeMarks: Joi.number().empty(['', 'null', 'undefined', null]).min(0).default(0),
  maxAttempts: Joi.number().empty(['', 'null', 'undefined', null]).min(1).default(1),
  difficulty: Joi.string().valid('easy', 'medium', 'hard').default('medium'),
  testType: Joi.string().valid('practice', 'mock', 'exam').default('practice'),
  startDate: Joi.date().optional().allow('', null, 'null', 'undefined'),
  endDate: Joi.date().optional().allow('', null, 'null', 'undefined'),
  scheduleAt: Joi.date().optional().allow('', null, 'null', 'undefined'),
  scheduledStartTime: Joi.date().optional().allow('', null, 'null', 'undefined'),
  scheduledEndTime: Joi.date().optional().allow('', null, 'null', 'undefined'),
  language: Joi.string().valid('hi', 'en', 'both').default('hi'),
  status: Joi.string().valid('draft', 'active', 'inactive').default('draft'),
}).custom(validateTestMarks)

const updateCourseTestSchema = Joi.object({
  course: Joi.string().hex().length(24).optional(),
  courseId: Joi.string().hex().length(24).optional(),
  subjects: Joi.array().items(Joi.string().hex().length(24)).optional(),
  topics: Joi.array().items(Joi.string().hex().length(24)).optional(),
  chapters: Joi.array().items(Joi.string().hex().length(24)).optional(),
  title: Joi.string().trim(),
  slug: Joi.string().trim(),
  description: Joi.string().trim().optional().allow('', null),
  instruction: Joi.string().trim().optional().allow('', null),
  instructionsNew: Joi.string().trim().optional().allow('', null),
  image: Joi.string().trim().optional().allow('', null),
  duration: Joi.number().empty(['', 'null', 'undefined', null]).min(1),
  sortOrder: Joi.number().empty(['', 'null', 'undefined', null]).integer().min(0),
  totalQuestions: Joi.number().empty(['', 'null', 'undefined', null]).min(0),
  totalMarks: Joi.number().empty(['', 'null', 'undefined', null]).min(0),
  passingMarks: Joi.number().empty(['', 'null', 'undefined', null]).min(0).default(0),
  marksPerQuestion: Joi.number().empty(['', 'null', 'undefined', null]).min(1),
  negativeMarks: Joi.number().empty(['', 'null', 'undefined', null]).min(0).default(0),
  maxAttempts: Joi.number().empty(['', 'null', 'undefined', null]).min(1),
  difficulty: Joi.string().valid('easy', 'medium', 'hard'),
  testType: Joi.string().valid('practice', 'mock', 'exam'),
  startDate: Joi.date().optional().allow('', null, 'null', 'undefined'),
  endDate: Joi.date().optional().allow('', null, 'null', 'undefined'),
  scheduleAt: Joi.date().optional().allow('', null, 'null', 'undefined'),
  scheduledStartTime: Joi.date().optional().allow('', null, 'null', 'undefined'),
  scheduledEndTime: Joi.date().optional().allow('', null, 'null', 'undefined'),
  language: Joi.string().valid('hi', 'en', 'both'),
  status: Joi.string().valid('draft', 'active', 'inactive'),
}).min(1)

const listCourseTestQuerySchema = Joi.object({
  status: Joi.string().valid('draft', 'active', 'inactive'),
  course: Joi.string().hex().length(24),
  subject: Joi.string().hex().length(24),
  topic: Joi.string().hex().length(24),
  chapter: Joi.string().hex().length(24),
  search: Joi.string().trim().max(200),
  sortBy: Joi.string().valid('createdAt', 'sortOrder').default('sortOrder'),
  order: Joi.string().valid('asc', 'desc').default('asc'),
  page: Joi.number().integer().min(1).default(1),
  limit: Joi.number().integer().min(1).max(100).default(20),
})

const bulkCreateCourseTestSchema = Joi.array().items(
  Joi.object({
    course: Joi.string().hex().length(24).required(),
    subjects: Joi.array().items(Joi.string().hex().length(24)).optional(),
    topics: Joi.array().items(Joi.string().hex().length(24)).optional(),
    chapters: Joi.array().items(Joi.string().hex().length(24)).optional(),
    title: Joi.string().trim().required(),
    slug: Joi.string().trim().optional(),
    description: Joi.string().trim().optional().allow('', null),
    instruction: Joi.string().trim().optional().allow('', null),
    instructionsNew: Joi.string().trim().optional().allow('', null),
    image: Joi.string().trim().optional().allow('', null).default(''),
    duration: Joi.number().empty(['', 'null', 'undefined', null]).min(1).required(),
    sortOrder: Joi.number().empty(['', 'null', 'undefined', null]).integer().min(0).default(0),
    totalQuestions: Joi.number().empty(['', 'null', 'undefined', null]).min(0).default(0),
    totalMarks: Joi.number().empty(['', 'null', 'undefined', null]).min(0).default(0),
    passingMarks: Joi.number().empty(['', 'null', 'undefined', null]).min(0).default(0),
    marksPerQuestion: Joi.number().empty(['', 'null', 'undefined', null]).min(1).default(1),
    negativeMarks: Joi.number().empty(['', 'null', 'undefined', null]).min(0).default(0),
    maxAttempts: Joi.number().empty(['', 'null', 'undefined', null]).min(1).default(1),
    difficulty: Joi.string().valid('easy', 'medium', 'hard').default('medium'),
    testType: Joi.string().valid('practice', 'mock', 'exam', 'other').default('practice'),
    startDate: Joi.date().optional().allow(null, '', 'null', 'undefined'),
    endDate: Joi.date().optional().allow(null, '', 'null', 'undefined'),
    scheduleAt: Joi.date().optional().allow(null, '', 'null', 'undefined'),
    language: Joi.string().valid('hi', 'en', 'both').default('hi'),
    status: Joi.string().valid('draft', 'active', 'inactive').default('draft'),
  })
).min(1)

module.exports = { createCourseTestSchema, updateCourseTestSchema, listCourseTestQuerySchema, bulkCreateCourseTestSchema }
