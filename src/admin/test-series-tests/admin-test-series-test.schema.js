...
const bulkCreateTestSeriesTestSchema = Joi.array().items(
  Joi.object({
    testSeries: Joi.string().hex().length(24).required(),
    subjectIds: Joi.array().items(objectId).single().default([]),
    chapterIds: Joi.array().items(objectId).single().default([]),
    topicIds: Joi.array().items(objectId).single().default([]),
    title: Joi.string().trim().required(),
...
module.exports = { createTestSeriesTestSchema, updateTestSeriesTestSchema, bulkCreateTestSeriesTestSchema }
