...
const bulkCreate = catchAsync(async (req, res) => {
    ...
    for (const rawRow of rawRows) {
      ...
      const dataRow = {
        testSeries: common.testSeries || common.testSeriesId,
    ...
    const { bulkCreateTestSeriesTestSchema } = require('./admin-test-series-test.schema')
    const { error, value } = bulkCreateTestSeriesTestSchema.validate(payloadArray, {
      abortEarly: false,
      stripUnknown: true,
      convert: true,
    })
    ...
    const created = await TestSeriesTest.create(finalDocs)
